    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden" style={{ background: '#08080F' }}>
      ...
      <div className="w-full max-w-md mx-4 animate-slide-up">
        <div className="rounded-2xl p-8 sm:p-10" style={{ background: '#16161F', border: '1px solid #252530' }}>
          ...
          <p style={{ color: '#6B7280', fontSize: 13 }} className="mb-8">Start tracking income and expenses in seconds</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {[
              { label: 'Full Name', id: 'name', type: 'text', value: name, onChange: setName, placeholder: 'John Doe' },
              { label: 'Email', id: 'email', type: 'email', value: email, onChange: setEmail, placeholder: 'you@example.com' },
              { label: 'Password', id: 'password', type: 'password', value: password, onChange: setPassword, placeholder: '••••••••' },
            ].map(f => (
              <div key={f.id}>
          ...
          <p className="mt-6 text-center text-sm" style={{ color: '#6B7280' }}>
            ...
            <Link to="/login" className="font-bold" style={{ color: '#22C55E' }}>Sign In</Link>
          </p>
        </div>
      </div>
    </div>
